...
const log = (msg: string) => { console.log(msg); fs.appendFileSync(LOG_FILE, msg + "\n"); };
    ...
    try {
      ...
      if (existente) {
        ...
        await prisma.rendimentoDiario.update({
          ...
          data: { amount: existente.amount.plus(rendimento), base: existente.base.plus(base), rate: existente.rate.plus(rate).div(2) },
        });
      } else {
      ...
      await prisma.user.update({ where: { id: investimento.userId }, data: { saldo: investimento.user.saldo.add(rendimento) } });
      await prisma.investimento.update({ where: { id: investimento.id }, data: { rendimentoAcumulado: investimento.rendimentoAcumulado.add(rendimento) } });
