...
export async function POST() {
  try {
    ...
    if (saldo.lte(0))
      return NextResponse.json(
        { error: "Saldo insuficiente para reinvestir" },
        { status: 400 }
      );

    // Reinvestimento em transação
    await prisma.$transaction(async (tx) => {
      await tx.investimento.create({
        data: {
          userId,
          valor: saldo.toNumber(),
          percentualDiario: new Decimal(0.015), // ✅ Ajuste para 1.5%
    ...
    return NextResponse.json({
      ...
      valorReinvestir: saldo.toNumber(),
    });
  } catch (error) {
